...
export function registerElearningStudio(app,{auth,aiLimit,readDb,writeDb,uploadDir,persistUploads=async()=>{}}){
  const base='/api/classes/:classId/elearning-studio';
  const check=async req=>{const db=(await readDb()),c=db.classes.find(c=>c._id===req.params.classId);if(!c)fail(404,'Không tìm thấy lớp.');if(req.user.role!=='admin'&&!(req.user.role==='teacher'&&c.teacherId===req.user._id))fail(403,'Chỉ giáo viên phụ trách lớp được soạn bài.');return {db,c};};
  ...
  app.get(base,auth(),access,async (req,res)=>res.json({drafts:(await readDb()).lessonDrafts.filter(d=>d.classId===req.params.classId)}));
  app.post(base,auth(),access,async (req,res)=>{const {db}=(await check(req)),document=validateSlideDocument(req.body);const d={_id:randomUUID(),classId:req.params.classId,createdBy:req.user._id,document,images:[],revision:1,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};db.lessonDrafts.push(d);(await writeDb(db));res.status(201).json({draft:d});});
  app.patch(base+'/:draftId',auth(),access,async (req,res)=>{const {db}=(await check(req)),d=find(db,req);if(req.body.revision!==d.revision)fail(409,'Bản nháp đã thay đổi ở cửa sổ khác. Mở lại bản đã lưu trước khi sửa tiếp.');d.document=validateSlideDocument(req.body.document,d.images);d.revision++;d.updatedAt=new Date().toISOString();(await writeDb(db));res.json({draft:d});});
  app.post(base+'/:draftId/images',auth(),access,async (req,res,next)=>{find((await readDb()),req);upload(req,res,e=>e?res.status(400).json({error:'Chọn một ảnh PNG/JPEG/WebP tối đa 10 MB.'}):next());},async (req,res)=>{
    const b=req.file?.buffer;const ext=b?.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10]))?'png':b?.[0]===255&&b?.[1]===216&&b?.[2]===255?'jpg':b?.toString('ascii',0,4)==='RIFF'&&b?.toString('ascii',8,12)==='WEBP'?'webp':null;if(!ext)fail(400,'Ảnh không hợp lệ.');const {db}=(await check(req)),d=find(db,req);if(d.images.length>=150)fail(400,'Mỗi bài tối đa 150 ảnh.');const name=`lesson-image-${randomUUID()}.${ext}`,file=path.join(uploadDir,name),url='/uploads/'+name;fs.writeFileSync(file,b);try{d.images.push(url);(await writeDb(db));}catch(e){fs.unlinkSync(file);throw e;}res.json({url});
  });
  app.post(base+'/:draftId/questions',auth(),access,aiLimit,async(req,res)=>{
    const d=find((await readDb()),req),count=boundedCount(req.body.count,3,5),context=d.document.slides.flatMap(s=>s.items.filter(o=>o.type==='text').map(o=>o.text)).join('\n');
    ...
    (await check(req));res.json({questions});
  });
  ...
  app.post(base+'/:draftId/generate-lesson',auth(),access,aiLimit,async(req,res)=>{
    const source=find((await readDb()),req),{topic,content='',images=[],theme='ocean'}=req.body;
    ...
    const {db}=(await check(req));find(db,req);
    ...
    db.lessonDrafts.push(draft);(await writeDb(db));res.status(201).json({draft});
  });
  app.post(base+'/:draftId/preview',auth(),access,async (req,res)=>{const d=find((await readDb()),req);const url=render(d);await persistUploads();res.json({url});});
  app.post(base+'/:draftId/publish',auth(),access,async (req,res)=>{
    const {db,c}=(await check(req)),d=find(db,req);if(req.body.revision!==d.revision)fail(409,'Bài giảng vừa thay đổi, hãy lưu lại trước khi xuất bản.');const assign=req.body.assign===true;
    ...
    try{let course=db.elearnings.find(e=>e._id===d.courseId);if(!course){course={_id:randomUUID(),createdAt:now,createdBy:req.user._id,classId:c._id,category:db.categories.find(v=>v.isActive!==false)?._id||'',author:req.user.fullName,viewCount:0,order:0,thumbnail:''};db.elearnings.push(course);d.courseId=course._id;}
      ...
      d.publishedAt=now;(await writeDb(db));res.json({draft:d,course,assignment});
