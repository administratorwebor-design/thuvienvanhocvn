...
try{
 ...
 // Newly seeded materials must remain manageable by admins even when the
 // separate permission-list response is stale or unavailable.
 const admin=await browser.newPage();
 await admin.route('**/api/teacher-library/elearnings',route=>route.fulfill({contentType:'application/json',body:JSON.stringify({ids:[]})}));
 await admin.goto(s.base+'/login');await admin.getByPlaceholder('username').fill('admin');await admin.locator('input[type=password]').fill(s.env.ADMIN_PASSWORD);await admin.locator('button[type=submit]').click();await admin.waitForURL(u=>u.pathname!=='/login',{waitUntil:'domcontentloaded'});
 const seeded=store.readDb();seeded.elearnings.push({_id:'premium-admin-regression',title:'Bài mẫu mới thêm',isActive:true,url:'/reference/gio-lanh-elearning/index.html',storyPath:'/reference/gio-lanh-elearning/index.html',category:s.fixtures.category._id,updatedAt:'2026-01-01T00:00:00.000Z'});store.writeDb(seeded);
 await admin.goto(s.base+'/elearnings');await admin.getByRole('button',{name:'Chỉnh sửa Bài mẫu mới thêm',exact:true}).click();await admin.getByRole('dialog').getByLabel('Tiêu đề',{exact:true}).fill('Bài mẫu đã sửa');await admin.getByRole('button',{name:'Lưu thay đổi',exact:true}).click();await admin.getByRole('dialog').waitFor({state:'hidden'});
 await admin.getByRole('button',{name:'Xóa Bài mẫu đã sửa',exact:true}).click();await admin.getByRole('button',{name:'Xác nhận xóa',exact:true}).click();await admin.getByRole('dialog').waitFor({state:'hidden'});assert.equal(store.readDb().elearnings.find(v=>v._id==='premium-admin-regression').isActive,false);
 assert.deepEqual(errors,[]);console.log('PASS: edit/delete/cancel all four menus, quiz answers, mobile dialogs, learner navigation, student visibility and admin management of newly seeded lessons with an empty permission list.');
